...
class NodeView(discord.ui.View):
    ...
    def build_node_embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(title=self.node["title"], description=self.node["description"], color=0xe74c3c)
        ...
        embed.add_field(name="Shop XP",   value=f"{self.player.xp or 0} / {next_threshold} (Level {self.player.shop_level or 1})", inline=True)
    ...
    async def _handle_choice(self, interaction: discord.Interaction, choice: str):
        ...
        levelled_up = False
        new_level   = self.player.shop_level or 1
        sp          = None

        if node_xp > 0:
            sp = self.session.query(SkillPoints).filter_by(player_id=self.player.id).first()
            levelled_up, new_level = apply_xp_and_check_levelup(self.player, sp, node_xp)
        ...
        # Post level-up message to TMC channel
        if levelled_up:
        ...
        result_embed.add_field(name="Strikes", value=f"{'X' * self.run.strikes}{'O' * (MAX_STRIKES - self.run.strikes)}", inline=True)
        if node_xp > 0:
            result_embed.add_field(name="Shop XP", value=f"+{node_xp} XP  |  {self.player.xp} / {next_threshold} (Level {self.player.shop_level})", inline=True)
        if levelled_up:
            result_embed.add_field(name="Level Up!", value=f"Bizard has sent you a message!", inline=False)
    ...
    async def _end_run(self, interaction, outcome, loot_line, result: str):
        ...
        next_threshold = SHOP_LEVEL_THRESHOLDS.get(self.player.shop_level or 1, 999)
        embed.add_field(name="Shop XP", value=f"{self.player.xp or 0} / {next_threshold} (Level {self.player.shop_level or 1})", inline=False)
