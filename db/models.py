...
class ActiveRun(Base):
    ...
    node_sequence   = Column(Text)          # JSON array of node IDs
    loot_this_run   = Column(Text)          # JSON array of item IDs
...
class Quest(Base):
    ...
    status          = Column(Text, default="active")    # active / complete / failed
    created_at      = Column(DateTime, default=datetime.utcnow)
...
class RunHistory(Base):
    ...
    player_id       = Column(Integer, ForeignKey("players.id"), nullable=False)
    dungeon_id      = Column(Text, nullable=False)
    outcome         = Column(Text, nullable=False)      # completed / fled / died
    nodes_completed = Column(Integer, default=0)
    items_recovered = Column(Text)                      # JSON array
